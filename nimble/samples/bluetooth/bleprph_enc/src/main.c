...
bleprph_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ...
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            MODLOG_DFLT("Passkey on device's display: %" PRIu32 , event->passkey.params.numcmp);
            MODLOG_DFLT("Accept or reject the passkey through console in this format -> key Y or key N");
            pkey.action = event->passkey.params.action;
            ...
            if (scli_receive_key(&key)) {
                ...
                MODLOG_DFLT("Timeout! Rejecting the key");
            }
            #endif
           ...
           /*shell shoud support input num here*/
            ...
            MODLOG_DFLT("Enter the passkey through console in this format-> key 123456");
            pkey.action = event->passkey.params.action;
            ...
            if (scli_receive_key(&key)) {
                ...
                pkey.passkey = 0;
                MODLOG_DFLT("Timeout! Passing 0 as the key");
            }
            #endif
