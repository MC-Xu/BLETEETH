...
static void spi_tx_callback(SPI_CbFlagOpt flag,uint16_t* pBuf,uint16_t size)
{
	...
	printf("slave send done! sent %d data as interrupt mode\n", size);
}

static void spi_rx_callback(SPI_CbFlagOpt flag,uint16_t* pBuf,uint16_t size)
{
    slave_receive_done_flag = true;
	printf("slave receive done! receive %d data as interrupt mode\n", size);
}
...
void spi_slave_receive_send_interrupt_test(void)
{
	...
	printf("slave turn to send mode \n");
	HAL_SPI_Init(&SPI0_OBJ);
	/* Transmit data */
	...
	printf("slave receive data: \n");
	for (uint16_t cnt = 0; cnt < slave_receive_send_cnt; cnt++) {
		printf("0x%04x,", slave_receive_buf[cnt]);
		if (cnt % 0x10 == 0xf) {
			printf("\n");
		}
	}
	...
	for (uint16_t cnt = 0; cnt < slave_receive_send_cnt; cnt++) {
		printf("0x%04x,", slave_send_buf[cnt]);
		if (cnt % 0x10 == 0xf) {
			printf("\n");
		}
	}
	
	printf("slave turn to quasi mode \n");
	HAL_SPI_Init(&SPI0_OBJ);
	/* Receive data */
	for (uint16_t cnt = 0; cnt < slave_receive_send_cnt; cnt++) {
